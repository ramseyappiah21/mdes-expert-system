%==============================================================================
% HTTP-free JSON API core — used by the Prolog web server and Vercel WASM
%==============================================================================
...
adv_prereqs_out(In, _{course: Course, title: Title, ancestors: Nodes, longest_path: Depth}) :-
    need(In, course, Course),
...
adv_forward_out(In, _{student: Student, steps: Steps, profile: Profile}) :-
    need(In, student, Student),
    student(Student),
...
adv_whatif_out(In, _{student: Student, passed: Extra, unlocked: Rows}) :-
    need(In, student, Student),
...
dispatch_api('GET', '/api/health', _, 200, _{ok: true, system: 'MDES', domains: 10, runtime: vercel}).
dispatch_api('GET', '/api/options', _, 200, Out) :-
    options_dict(Out).
dispatch_api('POST', '/api/advising/sample', In, 200, Out) :-
    adv_sample_out(In, Out).
dispatch_api('POST', '/api/advising/consult', In, 200, Out) :-
    adv_consult_out(In, Out).
dispatch_api('POST', '/api/advising/eligible', In, 200, Out) :-
    adv_eligible_out(In, Out).
dispatch_api('POST', '/api/advising/prereqs', In, 200, Out) :-
    adv_prereqs_out(In, Out).
dispatch_api('POST', '/api/advising/forward', In, 200, Out) :-
    adv_forward_out(In, Out).
dispatch_api('POST', '/api/advising/whatif', In, 200, Out) :-
    adv_whatif_out(In, Out).
dispatch_api('POST', Path, In, 200, Out) :-
    atom_concat('/api/', Rest, Path),
    atomic_list_concat([Domain0, Action0], '/', Rest),
    atomize(Domain0, Domain),
    atomize(Action0, Action),
    Domain \== advising,
    domain_action(Action, Domain, In, Out).

domain_action(assess, Domain, In, Out) :-
    domain_assess(Domain, In, Out).
domain_action(backward, Domain, In, Out) :-
    domain_backward(Domain, In, Out).
domain_action(recursive, Domain, In, Out) :-
    domain_recursive(Domain, In, Out).
domain_action(forward, Domain, In, Out) :-
    domain_forward(Domain, In, Out).

normalize_method(Raw, Method) :-
    atomize(Raw, A0),
    upcase_atom(A0, Method).

normalize_path(Raw, Path) :-
    atomize(Raw, A0),
    atomic_list_concat([NoQuery|_], '?', A0),
    (   sub_atom(NoQuery, _, 1, 0, '/')
    ->  sub_atom(NoQuery, 0, _, 1, Trimmed0)
    ;   Trimmed0 = NoQuery
    ),
    (   sub_atom(Trimmed0, 0, 1, _, '/')
    ->  Path = Trimmed0
    ;   atom_concat('/', Trimmed0, Path)
    ).

dispatch_files(Method0, Path0, InPath, OutPath, Status) :-
    normalize_method(Method0, Method),
    normalize_path(Path0, Path),
    setup_call_cleanup(open(InPath, read, InS, [type(text)]),
                       json_read_dict(InS, In),
                       close(InS)),
    catch(
        (   dispatch_api(Method, Path, In, Status, Out)
        ->  true
        ;   Status = 404,
            Out = _{error: 'Unknown API route'}
        ),
        Error,
    ...
    Out2 = Out,
    setup_call_cleanup(open(OutPath, write, OutS, [type(text)]),
                       json_write_dict(OutS, Out2),
                       close(OutS)).
